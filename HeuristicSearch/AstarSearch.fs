...
let astar_search (iface : DomainInterface.CostHeuristicDuplicateDomainInterface<float, 'state, 'hashvalue>) =
    ...
    let consider_child current_node (state, step_cost) =
        let child_node = { parent = Some current_node; state = state; cost = current_node.cost + step_cost; f = current_node.cost + (iface.H state) }
