...
let greedy_best_first_search (iface : DomainInterface.CostHeuristicDuplicateDomainInterface<float, 'state, 'hashvalue>) =
    ...
    let root = make_root iface.H iface.InitialState
    ...
    let consider_child current_node (state, step_cost) =
        let child_node = { parent = Some current_node; state = state; cost = current_node.cost + step_cost; heuristic_value = iface.H state }
